...
namespace myvk {
	...
	Buffer::Create(const std::shared_ptr<Device> &device, VkDeviceSize size, VmaMemoryUsage memory_usage,
	...
	std::shared_ptr<Buffer>
	Buffer::Create(const std::shared_ptr<Device> &device, const VkBufferCreateInfo &create_info,
		...
		for (auto &i : access_queues) queue_family_set.insert(i->GetFamilyIndex());
		...
		if (vmaCreateBuffer(device->GetAllocatorHandle(), &new_info, &alloc_create_info,
							&ret->m_buffer, &ret->m_allocation,
							nullptr) != VK_SUCCESS)
			return nullptr;
	...
	std::shared_ptr<Buffer>
	Buffer::CreateStaging(const std::shared_ptr<Device> &device, VkDeviceSize size,
						  const std::vector<std::shared_ptr<Queue>> &access_queues) {
	...
	void Buffer::Unmap() const {
		vmaUnmapMemory(m_device_ptr->GetAllocatorHandle(), m_allocation);
	}
