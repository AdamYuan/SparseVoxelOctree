...
namespace myvk {
	...
	Swapchain::Create(const std::shared_ptr<Queue> &graphics_queue,
		...
		if (surface_formats.empty() || present_modes.empty()) return nullptr;
		...
		if (vkCreateSwapchainKHR(graphics_queue->GetDevicePtr()->GetHandle(), &create_info, nullptr,
								 &ret->m_swapchain) != VK_SUCCESS)
			return nullptr;

		//get image count
		vkGetSwapchainImagesKHR(graphics_queue->GetDevicePtr()->GetHandle(), ret->m_swapchain, &ret->m_image_count, nullptr);
	...
	VkResult Swapchain::AcquireNextImage(uint32_t *p_image_index, const std::shared_ptr<Semaphore> &signal_semaphore,
		...
		return vkAcquireNextImageKHR(GetDevicePtr()->GetHandle(),
									 m_swapchain, UINT64_MAX,
									 signal_semaphore ? signal_semaphore->GetHandle() : nullptr,
									 signal_fence ? signal_fence->GetHandle() : nullptr,
									 p_image_index);
	}
