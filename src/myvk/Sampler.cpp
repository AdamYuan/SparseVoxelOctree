...
namespace myvk {
	...
	Sampler::Create(const std::shared_ptr<Device> &device, const VkSamplerCreateInfo &create_info) {
	...
	std::shared_ptr<Sampler>
	Sampler::Create(const std::shared_ptr<Device> &device, VkFilter filter, VkSamplerAddressMode address_mode,
	...
	std::shared_ptr<Sampler>
	Sampler::CreateClampToBorder(const std::shared_ptr<Device> &device, VkFilter filter, VkBorderColor border_color,
								 VkSamplerMipmapMode mipmap_mode, uint32_t mipmap_level, bool request_anisotropy,
								 float max_anisotropy) {
		VkSamplerCreateInfo create_info = {};
		create_info.sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO;
