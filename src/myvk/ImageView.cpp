...
namespace myvk {
	...
	ImageView::Create(const std::shared_ptr<ImageBase> &image, VkImageViewType view_type, VkFormat format,
					  VkImageAspectFlags aspect_mask, uint32_t base_mip_level, uint32_t level_count,
					  uint32_t base_array_layer, uint32_t layer_count, const VkComponentMapping &components) {
	...
	std::shared_ptr<ImageView> ImageView::Create(const std::shared_ptr<SwapchainImage> &swapchain_image) {
		return Create(swapchain_image, VK_IMAGE_VIEW_TYPE_2D,
					  swapchain_image->GetSwapchainPtr()->GetImageFormat(),
	...
	std::shared_ptr<ImageView>
	ImageView::Create(const std::shared_ptr<ImageBase> &image, const VkImageViewCreateInfo &create_info) {
