...
namespace myvk {
	...
	void DeviceCreateInfo::fetch_queues(const std::shared_ptr<Device> &device) const {
		...
		for (uint32_t i = 0; i < m_out_queues.size(); ++i) {
			...
			if (queues.find(queue_id) != queues.end()) continue;
			if (m_out_present_queues[i]) {
				std::shared_ptr<PresentQueue> present_queue = PresentQueue::create(
					device, m_surface_ptrs[i],
					selections[i].presentQueueFamilyIndex,
					selections[i].presentQueueIndex);
		...
		for (uint32_t i = 0; i < m_out_queues.size(); ++i) {
			std::pair<uint32_t, uint32_t> queue_id = {selections[i].queueFamilyIndex,
													  selections[i].queueIndex};
			if (queues.find(queue_id) != queues.end()) continue;
			queues[queue_id].first = Queue::create(device,
												   selections[i].queueFamilyIndex,
												   selections[i].queueIndex);
		}

		for (uint32_t i = 0; i < m_out_queues.size(); ++i) {
			(*m_out_queues[i]) = queues[{selections[i].queueFamilyIndex,
										 selections[i].queueIndex}].first;
			if (m_out_present_queues[i])
				(*m_out_present_queues[i]) = queues[{selections[i].presentQueueFamilyIndex,
													 selections[i].presentQueueIndex}].second;
		}
	}
