...
namespace myvk {
	...
	bool Fence::Signaled() const {
		return vkGetFenceStatus(m_device_ptr->GetHandle(), m_fence) == VK_SUCCESS;
	}


	Fence::~Fence() {
		if (m_fence) vkDestroyFence(m_device_ptr->GetHandle(), m_fence, nullptr);
	}
	...
	VkResult Fence::Reset() const {
		return vkResetFences(m_device_ptr->GetHandle(), 1, &m_fence);
	}

	FenceGroup::FenceGroup(const std::initializer_list<std::shared_ptr<Fence>> &fences) {
		Initialize(fences);
	}

	FenceGroup::FenceGroup(const std::vector<std::shared_ptr<Fence>> &fences) {
		Initialize(fences);
	}
	...
	VkResult FenceGroup::Reset() const {
		return vkResetFences(m_device, m_fences.size(), m_fences.data());
	}
