...
namespace myvk {
	std::shared_ptr<QueryPool>
	QueryPool::Create(const std::shared_ptr<Device> &device, VkQueryType query_type, uint32_t query_count,
	...
	VkResult
	QueryPool::GetResults(uint32_t first_query, uint32_t query_count, size_t data_size, void *data, VkDeviceSize stride,
