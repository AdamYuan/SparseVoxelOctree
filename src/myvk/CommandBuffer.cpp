...
namespace myvk {
	...
	CommandBuffer::Create(const std::shared_ptr<CommandPool> &command_pool,
		...
		if (vkAllocateCommandBuffers(
			command_pool->GetDevicePtr()->GetHandle(), &alloc_info,
			&ret->m_command_buffer) != VK_SUCCESS)
			return nullptr;
		return ret;
	}
	...
	CommandBuffer::~CommandBuffer() {
		if (m_command_buffer) {
			vkFreeCommandBuffers(
				m_command_pool_ptr->GetQueuePtr()->GetDevicePtr()->GetHandle(),
				m_command_pool_ptr->GetHandle(), 1, &m_command_buffer);
		}
	}

	VkResult CommandBuffer::Submit(const SemaphoreStageGroup &wait_semaphores,
								   const SemaphoreGroup &signal_semaphores,
	...
	void CommandBuffer::CmdBeginRenderPass(const std::shared_ptr<RenderPass> &render_pass,
										   ...
										   const VkOffset2D &offset, VkSubpassContents subpass_contents) const {
	...
	void
	CommandBuffer::CmdDrawIndexed(uint32_t index_count, uint32_t instance_count, uint32_t first_index,
	...
	void CommandBuffer::CmdPipelineBarrier(VkPipelineStageFlags src_stage, VkPipelineStageFlags dst_stage,
		...
		vkCmdPipelineBarrier(m_command_buffer, src_stage, dst_stage, 0,
	...
	void CommandBuffer::CmdGenerateMipmap2D(const std::shared_ptr<ImageBase> &image,
		...
		if (image->GetMipLevels() == 1) {
			...
			CmdPipelineBarrier(src_stage, dst_stage,
							   {},
							   {},
							   image->GetMemoryBarriers({subresource},
		...
		for (uint32_t i = 1; i < image->GetMipLevels(); ++i) {
			...
			CmdPipelineBarrier(i == 1 ? src_stage : VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, {},
							   {}, image->GetMemoryBarriers({blit.srcSubresource},
															i == 1 ? src_access_mask : VK_ACCESS_TRANSFER_WRITE_BIT,
															VK_ACCESS_TRANSFER_READ_BIT,
															i == 1 ? old_layout : VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
															VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL));
			CmdPipelineBarrier(i == 1 ? src_stage : VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, {},
							   {}, image->GetMemoryBarriers({blit.dstSubresource},
															i == 1 ? src_access_mask : 0,
