...
namespace myvk {
	...
	std::shared_ptr<DescriptorSet> DescriptorSet::Create(const std::shared_ptr<DescriptorPool> &descriptor_pool,
		...
		if (vkAllocateDescriptorSets(descriptor_pool->GetDevicePtr()->GetHandle(), &alloc_info,
									 &ret->m_descriptor_set) != VK_SUCCESS)
			return nullptr;
		return ret;
	}
