...
public:
	...
	const std::shared_ptr<Semaphore> &GetAcquireDoneSemaphorePtr() const {
		return m_acquire_done_semaphores[m_current_frame];
	};

	const std::shared_ptr<Semaphore> &GetRenderDoneSemaphorePtr() const {
		return m_render_done_semaphores[m_current_frame];
	};
