...
namespace myvk {
	...
	public:
		...
		Create(const std::shared_ptr<Device> &device, VmaMemoryUsage memory_usage, const VkImageCreateInfo &create_info,
		...
		static std::shared_ptr<Image>
		CreateTexture2D(const std::shared_ptr<Device> &device,
						const VkExtent2D &size, uint32_t mip_level, VkFormat format, VkImageUsageFlags usage,
