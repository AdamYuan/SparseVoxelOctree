...
namespace myvk {
	...
	public:
		...
		std::vector<VkImageMemoryBarrier> GetDstMemoryBarriers(
		...
		std::vector<VkImageMemoryBarrier> GetMemoryBarriers(
		...
		VkImageMemoryBarrier GetDstMemoryBarrier(
		...
		VkImageMemoryBarrier GetMemoryBarrier(
			const VkImageSubresourceLayers &region,
			VkAccessFlags src_access_mask, VkAccessFlags dst_access_mask,
			VkImageLayout old_layout, VkImageLayout new_layout,
			const std::shared_ptr<myvk::Queue> &src_queue = nullptr,
			const std::shared_ptr<myvk::Queue> &dst_queue = nullptr) const;
