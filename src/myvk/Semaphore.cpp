...
namespace myvk {
	std::shared_ptr<Semaphore> Semaphore::Create(const std::shared_ptr<Device> &device) {
		...
		if (vkCreateSemaphore(device->GetHandle(), &info, nullptr,
							  &ret->m_semaphore) != VK_SUCCESS)
			return nullptr;
		return ret;
	}
	...
	SemaphoreGroup::SemaphoreGroup(
		const std::initializer_list<std::shared_ptr<Semaphore>> &semaphores) {
	...
	void SemaphoreGroup::Initialize(
		const std::vector<std::shared_ptr<Semaphore>> &semaphores) {
		m_semaphores.clear();
	...
	void SemaphoreStageGroup::Initialize(
		const std::vector<std::pair<std::shared_ptr<Semaphore>, VkPipelineStageFlags>>
		&stage_semaphores) {
		m_semaphores.clear();
		m_stages.clear();
